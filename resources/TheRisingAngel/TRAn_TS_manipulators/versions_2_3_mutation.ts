...
var equips: Array<EquipItem_V02> | null = JSON.parse(
  fs.readFileSync("./equipItems_V2.json", { encoding: "utf-8" }),
) as Array<EquipItem_V02>;
...
equips.forEach((e, _, __) => {
  if (newEquipSet![e.name]) {
    ...
    newEquipSet![e.name] = {
      ...
      attributesModifiers: e.attributesModifiers,
    } as EquipItem_V03;
  }
});
...
var newEquips: Array<EquipItem_V03> = Object.keys(newEquipSet!).map(
  (nameNE, _, __) => newEquipSet![nameNE],
);
newEquipSet = null;

fs.writeFileSync("./equipItems.json", JSON.stringify(newEquips, null, 2));
