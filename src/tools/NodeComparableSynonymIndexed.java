...
public class NodeComparableSynonymIndexed extends NodeComparable.NodeComparableDefaultAlghoritms<SynonymSet>
	...
	public NodeComparableSynonymIndexed() {
		this(new String[] {});
	}

	public NodeComparableSynonymIndexed(String[] aaaa) {
		this(new SynonymSet(aaaa));
	}
	 ...
	 * simple map
	 */
	// protected Map<SynonymSet, NodeComparable<SynonymSet>>
	// childrenBySynonymsBackMap;
	// protected Set<NodeComparable<SynonymSet>> childrenBySynonyms; // the "node
	// children"
	...
	@Override
	public NodeComparableSynonymIndexed clone() {
		return new NodeComparableSynonymIndexed(this);
	}

	@Override
	public Comparator<SynonymSet> getKeyComparator() {
		return SynonymSet.COMPARATOR_SYNONYM_SET;
	}

	@Override
	public SortedSetEnhanced<NodeComparable<SynonymSet>> getChildrenNC() {
		return this.childrenBySynonyms;
	}

	@Override
	public SynonymSet getKeyIdentifier() {
	...
	public NodeComparable<SynonymSet> getChildNCMostSimilarTo(NodeComparable<SynonymSet> copy) {
		...
		cmN = cm.convertTo(COMPARATOR_NODE, e -> {
	...
	// return c;
	// // not fond? -> scan
	// iterChildren = ((MapTreeAVL<SynonymSet, NodeComparable<SynonymSet>>)
	// this.childrenBySynonymsBackMap).iterator();
	// // V1
	//// while (iterChildren.hasNext()) { / entryIter = iterChildren.next(); / if
	/// (entryIter.getKey().areIntersecting(key)) / return entryIter.getValue(); /
	/// } /**
	...
	public void removeAlternative(String t) {
		this.alternatives.removeAlternative(t);
	}

	public boolean hasAlternative(String t) {
		return this.alternatives.hasAlternative(t);
	}

	public boolean canBeIdentifiedBy(SynonymSet eg) {
		return this.alternatives.areIntersecting(eg);
	}
