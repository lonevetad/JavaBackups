...
public class ImageDisassembler {
	...
	public static void startTest() {
		...
		bi = readImage(path, name);
		if (bi != null) {
			a = dissasembleImage(bi, 0, 0, 193, 100, 10, 8);
			if (a != null) {
				if (a.length > 0) {
					System.out.print("salvo le sottoimmagini :");
					System.out.println(writeImages(a, pathDestination, "cat disassembled", extensionAccepted[0], true));
				} else {
	...
	protected static final String[] extensionAccepted = { "png", "jpg", "jpeg", "bmp", "gif" };
	...
	void inizializza() {
		...
		if (!initializeFinished) {
			...
			// jcbCounterAtEnd.setSelected(true);
			//
	...
	void adaptSize() {
		...
		// jlImageShow, jlImage, jlFolderDest, jlWidth, jlHeight, jlStartXOffset,
		// jlStartYOffset, jlXOffset,
		// jlYOffset;
	...
	void disassembleImage() {
		...
		if (jtaFolderDest.getText().length() > 0) {
			ok = writeImages(dissasembleImage(orig,
					/*
					 * Integer.parseInt(jtaStartXOffset.getText()),
					 * Integer.parseInt(jtaStartYOffset.getText()),
					 * Integer.parseInt(jtaWidth.getText()), Integer.parseInt(jtaHeight.getText()),
					 * Integer.parseInt(jtaXOffset.getText()),
					 * Integer.parseInt(jtaYOffset.getText()))//
					 */
					intValues[JTA_Meaning.xStart.index]//
			...
			), jtaFolderDest.getText(), this.jtaNomeImmagine.getText(), extensionAccepted[0],
	...
	void reloadImage() {
		if (path != null /* && jtaNomeImmagine.getText() != null */) {
			try {
				orig = readImage(path, jtaNomeImmagine.getText());
				if (orig != null) {
	...
	void resetPathImage() {
		...
		chooser.setFileFilter(new FileNameExtensionFilter("Images", extensionAccepted));
		//
	...
	public static final BufferedImage[] dissasembleImage(BufferedImage bi, int xstart, int ystart, int w, int h,
		...
		if (bi != null && w > 0 && h > 0 && xstart >= 0 && ystart >= 0 && xjump >= 0 && yjump >= 0) {
			...
			if (wbi >= (xstart + w) && hbi >= (ystart + h)) {
				...
				/*
				 * ArrayList<BufferedImage> buff = new ArrayList<BufferedImage>( (int)
				 * (((((double) wbi - xstart) / (w + xjump)) * (((double) hbi - ystart) / (h +
				 * yjump)))) + 1);
				 */
	...
	public static final boolean writeImages(BufferedImage[] a, String pathFolder, String name, String extensionNoDot,
		...
		if (ret) {
			...
			if (extensionNoDot.length() == 0) {
				extensionNoDot = extensionAccepted[0];
			} else {
				notFound = true;
				toLowCase = extensionNoDot.toLowerCase();
				for (i = 0; (i < extensionAccepted.length && notFound); i++) {
					notFound = !(toLowCase.contains(extensionAccepted[i]));
				}
				if (notFound) {
					extensionNoDot = extensionAccepted[0];
				} else {
	...
	public static BufferedImage readImage(String path, String nameNoExtension) {
		...
		if (path != null && nameNoExtension != null && nameNoExtension.length() > 0) {
			...
			while (ret == null && ++i < extensionAccepted.length) {
				pathCompleto = pathParziale + '.' + extensionAccepted[i];
